...
final readonly class TaskInstanceUpdateProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): mixed
    {
        ...
        $entity = $this->em->getRepository(TaskInstance::class)->find($id);
        if (!$entity instanceof TaskInstance) {
            ...
            $entity->setTaskTemplate($data->taskTemplate);

            $entity->setSprintInstance($data->sprintInstance);

            $entity->setPriority($data->priority);

            $entity->setStatus($data->status);

            $entity->setTypeTask($data->typeTask);

            $entity->setParentTask($data->parentTask);

            $entity->setDependency($data->dependency);
