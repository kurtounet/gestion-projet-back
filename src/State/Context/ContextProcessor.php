...
/**
 * Processor pour l'entité Context.
 *
 * À brancher dans la ressource API Platform :
 *  - via attribut : processor: App\State\Context\ContextProcessor::class
 *  - ou via config YAML/XML.
 *
 * Si tu utilises des DTO (CreateContextDto / UpdateContextDto / ContextResponseDto),
...
final class ContextProcessor implements ProcessorInterface
{
    ...
    {
        // Exemple très basique si $data est déjà une entité Context
        // À adapter si $data est un DTO :

        if ($operation instanceof CollectionOperationInterface) {
            // POST (création)
            if (!$data instanceof Context) {
                // TODO: hydrater une entité Context à partir d'un DTO
                // $entity = new Context(...);
