...
/**
 * Processor pour l'entité TaskTemplate.
 *
 * À brancher dans la ressource API Platform :
 *  - via attribut : processor: App\State\TaskTemplate\TaskTemplateProcessor::class
 *  - ou via config YAML/XML.
 *
 * Si tu utilises des DTO (CreateTaskTemplateDto / UpdateTaskTemplateDto / TaskTemplateResponseDto),
 * adapte la méthode process() pour transformer DTO <-> entité.
 */
final class TaskTemplateProcessor implements ProcessorInterface
{
    ...
    {
        // Exemple très basique si $data est déjà une entité TaskTemplate
        // À adapter si $data est un DTO :

        if ($operation instanceof CollectionOperationInterface) {
            // POST (création)
            if (!$data instanceof TaskTemplate) {
                // TODO: hydrater une entité TaskTemplate à partir d'un DTO
