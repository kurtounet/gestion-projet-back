...
/**
 * Processor pour l'entité ProjectTemplateSprintTemplate.
 *
 * À brancher dans la ressource API Platform :
 *  - via attribut : processor: App\State\ProjectTemplateSprintTemplate\ProjectTemplateSprintTemplateProcessor::class
 *  - ou via config YAML/XML.
 *
 * Si tu utilises des DTO (CreateProjectTemplateSprintTemplateDto / UpdateProjectTemplateSprintTemplateDto / ProjectTemplateSprintTemplateResponseDto),
 * adapte la méthode process() pour transformer DTO <-> entité.
 */
final class ProjectTemplateSprintTemplateProcessor implements ProcessorInterface
{
    ...
    {
        // Exemple très basique si $data est déjà une entité ProjectTemplateSprintTemplate
        // À adapter si $data est un DTO :

        if ($operation instanceof CollectionOperationInterface) {
            // POST (création)
            if (!$data instanceof ProjectTemplateSprintTemplate) {
                // TODO: hydrater une entité ProjectTemplateSprintTemplate à partir d'un DTO
